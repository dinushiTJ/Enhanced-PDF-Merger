...
const pnOptions = document.getElementById('pnOptions');
...
function computeXY(position, width, height, textWidth, margin) {
...
async function doPageNumbers() {
    ...
    progress.textContent = 'Adding page numbers...';
    pnBtn.disabled = true;

    try {
        const position = document.getElementById('pnPosition').value;
        const start = Math.max(0, parseInt(document.getElementById('pnStart').value, 10) || 1);
        const size = Math.max(6, parseInt(document.getElementById('pnSize').value, 10) || 12);
        const showTotal = document.getElementById('pnShowTotal').checked;
        ...
        pages.forEach((page, i) => {
            const num = start + i;
            const label = showTotal ? `Page ${num} of ${total}` : String(num);
            const { width, height } = page.getSize();
            const textWidth = font.widthOfTextAtSize(label, size);
            const { x, y } = computeXY(position, width, height, textWidth, 28);
        ...
        result.innerHTML = `
            <div class="result-card">
                ...
                <p class="result-sub">Page numbers added to ${currentPageCount} page${currentPageCount === 1 ? '' : 's'} (${formatBytes(blob.size)}).</p>
        ...
        result.innerHTML = `<div class="error">Adding page numbers failed: ${escapeHtml(e.message)}</div>`;
        result.style.display = 'block';
    } finally {
...
pnBtn.addEventListener('click', doPageNumbers);
